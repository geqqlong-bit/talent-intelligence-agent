...
export function resolveRemoteOpenAIConfig(runtime = {}) {
  ...
  const baseUrl = normalizeText(runtime.baseUrl || process.env.TALENT_INTEL_REMOTE_BASE_URL || 'https://ark.cn-beijing.volces.com/api/v3');
  const apiKey = normalizeText(runtime.apiKey || process.env.TALENT_INTEL_REMOTE_API_KEY);
  const model = normalizeText(runtime.model || process.env.TALENT_INTEL_DEFAULT_MODEL || 'ep-20250215175954-zztch'); // Volcano Endpoint
