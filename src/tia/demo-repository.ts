...
function buildCandidateReportRow(state: DemoState, candidateId: string): any {
...
export function createDemoRepository({
  defaultWebhookUrls = [],
  logger = console
}: Omit<RepositoryOptions, 'db'> = {}): TiaRepository {
  ...
  async function dbWrite(table: string, operation: 'insert' | 'update' | 'delete', data: Record<string, any> = {}): Promise<any> {
  ...
  function buildFunnelRows(candidates: Candidate[]): any[] {
  ...
  return {
    ...
    async getPositionContext(positionId: string) {
    ...
    async stageUpdate(candidateId: string, newStage: string, options: Record<string, any> = {}) {
      const candidate = getStateRow(state.candidates, 'id', candidateId);
    ...
    async listTouchRecords({
      ...
      limit = 20
    }: { positionId?: string, candidateId?: string, limit?: number } = {}) {
