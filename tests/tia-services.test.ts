...
import { createTiaServiceContainer } from '../src/tia/service-container.ts';
...
test('scanRisks returns actionable demo alerts', async () => {
  ...
  const result: any = await services.scanRisks({});

  assert.equal(result.alerts.some((alert: any) => alert.type === 'candidate_stuck' && alert.candidateName === '李云龙'), true);
  assert.equal(result.alerts.some((alert: any) => alert.type === 'offer_risk' && alert.candidateName === '张大牛'), true);
  assert.equal(result.alerts.some((alert: any) => alert.type === 'contract_expiring'), true);
});

test('assessCandidate persists structured assessment and returns similar cases', async () => {
  const services = await createServices();
  const assessment: any = await services.assessCandidate({
  ...
  assert.equal(assessment.similarCases.some((item: any) => item.name === '王码农'), true);

  const candidates = await services.listCandidates('22222222-2222-2222-2222-222222222222');
  const updated: any = candidates.find((item: any) => item.id === '33333333-3333-3333-3333-333333333331');
...
test('getWorkbench returns multi-view snapshot for the selected position', async () => {
  const services = await createServices();
  const result: any = await services.getWorkbench({
  ...
  assert.equal(result.bd.clients.some((client: any) => client.name === '快手电商'), true);
  ...
  assert.equal(result.aiWorkbench.actions.some((action: any) => action.id === 'jd_diagnosis'), true);
});
...
test('validateOutput detects missing risk section in assessment', async () => {
  const services = await createServices();
  const result: any = await services.validateOutput({
  ...
  assert.match(String(result.feedback), /Risk|风险/i);
});
...
test('browserExtract normalizes page content from liepin', async () => {
  const services = await createServices();
  const result: any = services.browserExtract({
    platform: 'liepin',
    pageContent: [
...
test('resumeImport normalizes and returns results in demo mode', async () => {
  const services = await createServices();
  const result: any = await services.resumeImport({
    platform: 'liepin',
